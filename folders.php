...
if(!empty($_SESSION['tempuser']))
{
  ...
  if($gender!="" && $userid!="" )
  {
    ...
    if($count1==0)
    {
      ...
      $path3 = "users/".$userid."/Messages/";
	  $path4 = "users/".$userid."/Comment/";
	  $path5 = "users/".$userid."/Notification/";
      mkdir($path, 0755, true);
      mkdir($path2, 0775, true);
      mkdir($path3, 0775, true);
      mkdir($path4, 0775, true);
	  mkdir($path5, 0775, true);

      $img_name=$_FILES['file']['name'];
      	$img_tmp_name=$_FILES['file']['tmp_name'];
      	$prod_img_path=$img_name;
      	move_uploaded_file($img_tmp_name,"users/".$userid."/Profile/".$prod_img_path);
        $result=mysqli_query($con,"insert into user_profile_pic(user_id,image) values('$userid','$img_name')");
		if (!result) {
